...
def _get_face_mesh_module():
    """Get FaceMesh module with compatibility for MediaPipe 0.10.x+"""
    # Try old API first (MediaPipe < 0.10)
    solutions = getattr(mp, "solutions", None)
    if solutions and hasattr(solutions, "face_mesh"):
        return solutions.face_mesh
    
    # Try importing from python.solutions (older versions)
    try:
    ...
    # MediaPipe 0.10.x+ - create a compatibility wrapper
    try:
        ...
        class FaceMeshCompat:
            """Compatibility wrapper for FaceLandmarker (0.10.x) to act like FaceMesh"""
            ...
            def __init__(self, static_image_mode=False, max_num_faces=1, 
                ...
                # Find model file - use downloaded model
                model_path = 'face_landmarker.task'
                if not os.path.exists(model_path):
                    # Try to find in mediapipe directory
                    mp_dir = os.path.dirname(mp_module.__file__)
                    potential_paths = [
                        os.path.join(mp_dir, 'modules', 'face_landmarker', 'face_landmarker.task'),
                        os.path.join(mp_dir, 'face_landmarker.task'),
                    ]
            ...
            def process(self, image):
                """Process image and return landmark results"""
                from mediapipe.tasks.python.vision.core.image import Image
                from mediapipe.tasks.python.vision.core.image import ImageFormat
                
                # Convert image to MediaPipe Image if needed
                ...
                # Convert to old-style result with multi_face_landmarks
                class OldStyleResult:
                    def __init__(self, face_landmarks):
                        self.multi_face_landmarks = face_landmarks
                
                # Convert FaceLandmarker results to old format
                if result and result.face_landmarks:
                    ...
                    for face in result.face_landmarks:
                        ...
                        old_landmarks.append(type('FaceLandmarks', (), {'landmark': landmarks})())
                    return OldStyleResult(old_landmarks)
                
                return OldStyleResult([])
        
        # Return a module-like object with FaceMesh class
        ...
        raise RuntimeError(
            f"MediaPipe FaceMesh unavailable. Error: {e}. "
            "Please install mediapipe: pip install mediapipe"
        )
